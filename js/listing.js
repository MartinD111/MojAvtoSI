document.addEventListener("DOMContentLoaded", () => {
    ...
    priceEl.textContent = `$${listing.price.toLocaleString('en-US')}`;
    descriptionEl.textContent = listing.description || "The seller did not provide a description.";
    sellerNameEl.textContent = listing.author || "Unknown seller";
    const allUsers = JSON.parse(localStorage.getItem('mojavto_users')) || [];
    const seller = allUsers.find(user => user.username === listing.author);
    sellerLocationEl.textContent = seller ? (seller.region || "Unknown location") : "Unknown location";
    ...
    const details = {
        ...
        [translate('spec_mileage')]: `${listing.mileage.toLocaleString('en-US')} mi`,
        [translate('spec_power')]: listing.power ? `${listing.power} HP` : 'N/A',
    ...
    if (listing.seatMaterial) {
        details[translate('spec_seat_material') || 'Seat material'] = listing.seatMaterial;
    }

    // Dodaj tip hibrida če obstaja
    if (listing.hybridType) {
        details[translate('spec_hybrid_type') || 'Hybrid type'] = listing.hybridType;
    }

    // Dodaj električne podatke če obstajajo
    if (listing.battery) {
        details[translate('spec_battery') || 'Battery'] = `${listing.battery} kWh`;
    }
    if (listing.range) {
        details[translate('spec_range') || 'Range'] = `${listing.range} mi`;
    }
    ...
    contactEmailBtn.addEventListener('click', () => {
        ...
        const body = encodeURIComponent(`Hello,\n\nI'm interested in your listing for ${listing.title}.\n\nBest regards`);
    ...
    function updateGallery() {
        ...
        mainImage.alt = `${listing.title} - image ${currentImageIndex + 1}`;
    ...
    function displaySimilarVehicles(targetListing) {
        ...
        if (similarVehicles.length === 0) {
            container.innerHTML = '<p>No similar listings.</p>';
            return;
        }
        ...
        similarVehicles.forEach(vehicle => {
            ...
            card.innerHTML = `
                <div class="card-image-container">
                    <img src="${vehicle.images?.exterior[0] || 'https://via.placeholder.com/300x180?text=Vehicle'}" alt="${vehicle.title}" />
                </div>
                <div class="card-body">
                    <h3 class="card-title">${vehicle.title}</h3>
                    <p class="card-details">${translate('spec_year')}: ${vehicle.year} | ${translate('spec_mileage')}: ${vehicle.mileage.toLocaleString('en-US')} mi</p>
                    <p class="card-price">$${vehicle.price.toLocaleString('en-US')}</p>
                </div>`;
