...
(taksonomija_avto_*.xlsx, 5 columns: Make, Model, Trim, Fuel Type,
Engine Capacity (cc)).

Output structure (per-trim variant objects):
  { Brand: { Model: [ { trim, fuel_type, engine_capacity_cc } ] } }
"""
import json, sys, os
...
SRC = sys.argv[1] if len(sys.argv) > 1 else \
    r"C:/Users/marti/OneDrive/Desktop/Vehicle Taxonomy/taksonomija_avto_2026-06-02.xlsx"
OUT = sys.argv[2] if len(sys.argv) > 2 else \
    r"c:/Users/marti/AMS d.o.o/MojAvto.si/public/json/brands_models_global.json"

df = pd.read_excel(SRC, header=0)
# Read only the first 5 columns — ignore Engine Configuration
df = df.iloc[:, :5]
...
for _, r in df.iterrows():
    ...
    fuel = r['fuel'].strip()
    if fuel:
        variant['fuel_type'] = FUEL_MAP.get(fuel.lower(), fuel)
