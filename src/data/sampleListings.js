export const sampleCars = [
    {
        id: 'car-premium-1',
        title: 'Porsche 911 GT3 PDK',
        subtitle: '4.0 (510 KM) - PDK, Clubsport, Carbon, Lift',
        price: '215.900 €',
        priceRaw: 215900,
        salePriceEur: 189900,
        year: '2023',
        mileage: '1.200 km',
        mileageKm: 1200,
        power: '375 kW (510 KM)',
        powerKw: 375,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Vrhunsko stanje, brez praske. Prvi lastnik, SLO poreklo.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Porsche Center Ljubljana',
        sellerImage: 'https://i.pravatar.cc/150?u=porsche',
        sellerType: 'dealer',
        segment: 'Sport',
        category: 'avto',
        make: 'Porsche',
        model: '911',
        variant: 'GT3 PDK',
        transmission: 'Avtomatski',
        driveType: 'Zadaj',
        color: 'Srebrna',
        doorsCount: 2,
        seatsCount: 2,
        images: {
            exterior: [
                'images/porsche.png',
                'https://images.unsplash.com/photo-1606152421802-db97b9c7a11b?auto=format&fit=crop&w=800&q=80'
            ],
            interior: []
        },
        imgCount: 24,
        isNew: true,
        engineCc: 3996,
        fuelL100km: 13.2,
        firstRegistration: '05/2023',
        previousOwnersCount: 1,
        equipment: [
            'ABS', 'ESP', 'Alarm', 'AirbagDriver', 'AirbagPassenger', 'AirbagSide', 'LED', 'Matrix', 
            'AdaptiveLight', 'SportSeats', 'Armrest', 'Climate2Zone', 'HeatedSeats', 'ParkSensorFront', 
            'ParkSensorRear', 'RearCamera', 'Navigation', 'Bluetooth', 'CarPlay', 'PremiumSound', 
            'SportSuspension', 'TirePressure', 'ServiceBook'
        ],
        customEquipment: [
            { value: 'Clubsport paket' },
            { value: 'Karbonska šoljkasta sedeža' },
            { value: 'Sistem za dvig prednje osi (Lift)' }
        ],
        status: 'active'
    },
    {
        id: 'car-premium-2',
        title: 'Tesla Model S Plaid',
        subtitle: 'Tri-Motor (1020 KM) - Autopilot, 21" Arachnid',
        price: '112.400 €',
        priceRaw: 112400,
        salePriceEur: 99900,
        year: '2024',
        mileage: '5.000 km',
        mileageKm: 5000,
        power: '750 kW (1020 KM)',
        powerKw: 750,
        fuel: 'Elektrika',
        condition: 'Novo vozilo',
        sellerNote: 'Na voljo takoj. Vključena brezplačna supercharge 1 leto.',
        location: { country: 'SI', region: 'Podravska' },
        seller: 'Tesla Slovenija',
        sellerImage: 'https://i.pravatar.cc/150?u=tesla',
        sellerType: 'dealer',
        segment: 'E-sedan',
        category: 'avto',
        make: 'Tesla',
        model: 'Model S',
        variant: 'Plaid',
        transmission: 'Avtomatski',
        driveType: '4x4',
        color: 'Črna',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: [
                'images/tesla.png',
                'https://images.unsplash.com/photo-1621007947382-bb3c3994e3fd?auto=format&fit=crop&w=800&q=80'
            ],
            interior: []
        },
        imgCount: 18,
        isNew: true,
        engineCc: 0,
        electricRangeKm: 637,
        fuelL100km: null,
        firstRegistration: '01/2024',
        previousOwnersCount: 1,
        status: 'active'
    },
    {
        id: 'car-premium-3',
        title: 'Audi RS6 Avant quattro',
        subtitle: '4.0 TFSI V8 quattro - Performance, Carbon, Bang&Olufsen',
        price: '148.500 €',
        priceRaw: 148500,
        year: '2022',
        mileage: '28.500 km',
        mileageKm: 28500,
        power: '441 kW (600 KM)',
        powerKw: 441,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'V tovarniški garanciji. Servis pravkar opravljen.',
        location: { country: 'SI', region: 'Gorenjska' },
        seller: 'Audi Sport Center',
        sellerImage: 'https://i.pravatar.cc/150?u=audi',
        sellerType: 'dealer',
        segment: 'E-wagon',
        category: 'avto',
        make: 'Audi',
        model: 'RS6',
        variant: 'Avant quattro',
        transmission: 'Avtomatski',
        driveType: '4x4',
        color: 'Siva',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: [
                'images/audi.png',
                'https://images.unsplash.com/photo-1555215695-3004980ad54e?auto=format&fit=crop&w=800&q=80'
            ],
            interior: []
        },
        imgCount: 32,
        isNew: false,
        engineCc: 3996,
        fuelL100km: 11.5,
        firstRegistration: '09/2022',
        previousOwnersCount: 1,
        status: 'active'
    },
    {
        id: 'car-1',
        title: 'Audi A4 Avant 2.0 TDI S-line',
        subtitle: '2.0 TDI S-tronic S-line (LED, NAVI, PANORAMA)',
        price: '24.500 €',
        priceRaw: 24500,
        year: '2019',
        mileage: '125.000 km',
        mileageKm: 125000,
        power: '140 kW (190 KM)',
        powerKw: 140,
        fuel: 'Diesel',
        condition: 'Rabljeno',
        sellerNote: 'Avto prodajam, ker kupujem večjega.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Janez Novak (Avtohiša Premium)',
        sellerImage: 'https://i.pravatar.cc/150?u=janez',
        sellerType: 'dealer',
        segment: 'D-sedan',
        category: 'avto',
        make: 'Audi',
        model: 'A4',
        variant: 'Avant 2.0 TDI S-line',
        transmission: 'Avtomatski',
        driveType: 'Spredaj',
        color: 'Siva',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: [
                'https://images.unsplash.com/photo-1606152421802-db97b9c7a11b?auto=format&fit=crop&w=800&q=80',
                'https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=800&q=80'
            ],
            interior: []
        },
        imgCount: 15,
        isNew: true,
        engineCc: 1968,
        fuelL100km: 5.2,
        equipment: [
            'ABS', 'ESP', 'Alarm', 'AirbagDriver', 'AirbagPassenger', 'AirbagSide', 'LED', 'Matrix', 
            'Climate2Zone', 'HeatedSeats', 'ParkSensorFront', 'ParkSensorRear', 'RearCamera', 
            'Navigation', 'Bluetooth', 'CarPlay', 'CruiseControl', 'TirePressure', 'ServiceBook'
        ],
        customEquipment: [
            { value: 'S-line zunanji in notranji paket' },
            { value: 'Virtualni števec' }
        ],
        status: 'active'
    },
    {
        id: 'car-2',
        title: 'Volkswagen Golf 1.5 TSI Evo Highline',
        subtitle: '1.5 TSI Evo Highline',
        price: '18.900 €',
        priceRaw: 18900,
        year: '2020',
        mileage: '65.000 km',
        mileageKm: 65000,
        power: '110 kW (150 KM)',
        powerKw: 110,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Prvi lastnik, potrjena servisna knjiga.',
        location: { country: 'SI', region: 'Podravska' },
        seller: 'Marko Kovač',
        sellerImage: 'https://i.pravatar.cc/150?u=marko',
        sellerType: 'private',
        segment: 'C-hatchback',
        category: 'avto',
        make: 'Volkswagen',
        model: 'Golf',
        variant: '1.5 TSI Evo Highline',
        transmission: 'Ročni',
        driveType: 'Spredaj',
        color: 'Bela',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: [
                'https://images.unsplash.com/photo-1621007947382-bb3c3994e3fd?auto=format&fit=crop&w=800&q=80',
                'https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=800&q=80'
            ]
        },
        imgCount: 8,
        isNew: false,
        engineCc: 1498,
        fuelL100km: 6.1,
        status: 'active'
    },
    {
        id: 'car-3',
        title: 'BMW Serija 3 320d xDrive M-Sport',
        subtitle: '320d xDrive M-Sport',
        price: '31.200 €',
        priceRaw: 31200,
        year: '2021',
        mileage: '48.000 km',
        mileageKm: 48000,
        power: '140 kW (190 KM)',
        powerKw: 140,
        fuel: 'Diesel',
        condition: 'Novo vozilo',
        sellerNote: 'Na voljo takoj, možen ugoden leasing.',
        location: { country: 'SI', region: 'Savinjska' },
        seller: 'Andrej Potočnik (BMW Center)',
        sellerImage: 'https://i.pravatar.cc/150?u=andrej',
        sellerType: 'dealer',
        segment: 'D-sedan',
        category: 'avto',
        make: 'BMW',
        model: 'Serija 3',
        variant: '320d xDrive M-Sport',
        transmission: 'Avtomatski',
        driveType: '4x4 (xDrive)',
        color: 'Modra',
        doorsCount: 4,
        seatsCount: 5,
        images: {
            exterior: [
                'https://images.unsplash.com/photo-1555215695-3004980ad54e?auto=format&fit=crop&w=800&q=80',
                'https://images.unsplash.com/photo-1606152421802-db97b9c7a11b?auto=format&fit=crop&w=800&q=80'
            ]
        },
        imgCount: 22,
        isNew: true,
        engineCc: 1995,
        fuelL100km: 4.8,
        status: 'active'
    },
    {
        id: 'car-4',
        title: 'Honda CR-V 2.0 i-MMD Hybrid Elegance',
        subtitle: '2.0 i-MMD Hybrid Elegance',
        price: '27.950 €',
        priceRaw: 27950,
        year: '2022',
        mileage: '22.300 km',
        mileageKm: 22300,
        power: '135 kW (184 KM)',
        powerKw: 135,
        fuel: 'Hibrid',
        condition: 'Testno vozilo',
        sellerNote: 'Nizka poraba, varčen hibridni motor.',
        location: { country: 'SI', region: 'Obalno-kraška' },
        seller: 'Uroš Zupan',
        sellerImage: 'https://i.pravatar.cc/150?u=uros',
        sellerType: 'private',
        segment: 'SUV',
        category: 'avto',
        make: 'Honda',
        model: 'CR-V',
        variant: '2.0 i-MMD Hybrid Elegance',
        transmission: 'Avtomatski',
        driveType: 'Spredaj',
        color: 'Rdeča',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: [
                'https://images.unsplash.com/photo-1568844293986-8d0400bc4745?auto=format&fit=crop&w=800&q=80',
                'https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=800&q=80'
            ]
        },
        imgCount: 12,
        isNew: false,
        engineCc: 1993,
        fuelL100km: 4.9,
        status: 'active'
    },
    {
        id: 'car-5',
        title: 'Mercedes-Benz C-Razred 220 d AMG Line 4MATIC',
        subtitle: 'AMG Line 4MATIC (Full LED, Panorama)',
        price: '38.700 €',
        priceRaw: 38700,
        year: '2022',
        mileage: '35.000 km',
        mileageKm: 35000,
        power: '147 kW (200 KM)',
        powerKw: 147,
        fuel: 'Diesel',
        condition: 'Oldtimer',
        sellerNote: 'Odlično ohranjen, garažiran od prvega dne.',
        location: { country: 'SI', region: 'Gorenjska' },
        seller: 'Luka Horvat (Star Premium)',
        sellerImage: 'https://i.pravatar.cc/150?u=luka',
        sellerType: 'dealer',
        segment: 'D-sedan',
        category: 'avto',
        make: 'Mercedes-Benz',
        model: 'C-Razred',
        variant: '220 d AMG Line 4MATIC',
        transmission: 'Avtomatski',
        driveType: '4x4 (4MATIC)',
        color: 'Črna',
        doorsCount: 4,
        seatsCount: 5,
        images: {
            exterior: [
                'https://images.unsplash.com/photo-1618843479313-40f8afb4b4d8?auto=format&fit=crop&w=800&q=80',
                'https://images.unsplash.com/photo-1606152421802-db97b9c7a11b?auto=format&fit=crop&w=800&q=80'
            ]
        },
        imgCount: 28,
        isNew: true,
        engineCc: 1950,
        fuelL100km: 5.5,
        status: 'active'
    },
    {
        id: 'car-6',
        title: 'Škoda Octavia 2.0 TDI Style',
        subtitle: '2.0 TDI Style (ACC, NAVI, LED)',
        price: '21.400 €',
        priceRaw: 21400,
        year: '2020',
        mileage: '98.000 km',
        mileageKm: 98000,
        power: '110 kW (150 KM)',
        powerKw: 110,
        fuel: 'Diesel',
        condition: 'Rabljeno',
        sellerNote: 'Avto prodajam, ker sem dobil službenega.',
        location: { country: 'SI', region: 'Goriška' },
        seller: 'Maja Bizjak',
        sellerImage: 'https://i.pravatar.cc/150?u=maja',
        sellerType: 'private',
        segment: 'C-hatchback',
        category: 'avto',
        make: 'Škoda',
        model: 'Octavia',
        variant: '2.0 TDI Style',
        transmission: 'Avtomatski',
        driveType: 'Spredaj',
        color: 'Srebrna',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: [
                'https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=800&q=80',
                'https://images.unsplash.com/photo-1606152421802-db97b9c7a11b?auto=format&fit=crop&w=800&q=80'
            ]
        },
        imgCount: 10,
        isNew: false,
        engineCc: 1968,
        fuelL100km: 4.7,
        status: 'active'
    },
    {
        id: 'car-7',
        title: 'Toyota RAV4 2.5 Hybrid AWD-i Executive',
        subtitle: '2.5 Hybrid AWD-i Executive',
        price: '34.900 €',
        priceRaw: 34900,
        year: '2021',
        mileage: '42.000 km',
        mileageKm: 42000,
        power: '163 kW (222 KM)',
        powerKw: 163,
        fuel: 'Hibrid',
        condition: 'Rabljeno',
        sellerNote: 'Odlično ohranjen, redno servisiran na pooblaščenem servisu.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Toyota Center Ljubljana',
        sellerImage: 'https://i.pravatar.cc/150?u=toyota',
        sellerType: 'dealer',
        segment: 'SUV',
        category: 'avto',
        make: 'Toyota',
        model: 'RAV4',
        variant: '2.5 Hybrid AWD-i Executive',
        transmission: 'Avtomatski',
        driveType: '4x4 (AWD-i)',
        color: 'Bela',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1590362891175-306fbc3b538e?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 18,
        isNew: true,
        engineCc: 2487,
        fuelL100km: 5.8,
        status: 'active'
    },
    {
        id: 'car-8',
        title: 'Ford Focus 1.0 EcoBoost Titanium',
        subtitle: '1.0 EcoBoost Titanium',
        price: '15.400 €',
        priceRaw: 15400,
        year: '2019',
        mileage: '78.000 km',
        mileageKm: 78000,
        power: '92 kW (125 KM)',
        powerKw: 92,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Varčen motor, veliko dodatne opreme.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Bojan Mlakar',
        sellerImage: 'https://i.pravatar.cc/150?u=bojan',
        sellerType: 'private',
        segment: 'C-hatchback',
        category: 'avto',
        make: 'Ford',
        model: 'Focus',
        variant: '1.0 EcoBoost Titanium',
        transmission: 'Ročni',
        driveType: 'Spredaj',
        color: 'Modra',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1551816230-ef5deaed4a26?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 5,
        isNew: false,
        engineCc: 998,
        fuelL100km: 5.6,
        status: 'active'
    },
    {
        id: 'car-9',
        title: 'Hyundai Tucson 1.6 T-GDI PHEV Premium',
        subtitle: '1.6 T-GDI PHEV Premium',
        price: '38.200 €',
        priceRaw: 38200,
        year: '2022',
        mileage: '15.600 km',
        mileageKm: 15600,
        power: '195 kW (265 KM)',
        powerKw: 195,
        fuel: 'Priključni hibrid',
        condition: 'Testno vozilo',
        sellerNote: 'Vrhunsko ohranjen, polna oprema, garancija.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Avtohiša Real',
        sellerImage: 'https://i.pravatar.cc/150?u=hyundai',
        sellerType: 'dealer',
        segment: 'SUV',
        category: 'avto',
        make: 'Hyundai',
        model: 'Tucson',
        variant: '1.6 T-GDI PHEV Premium',
        transmission: 'Avtomatski',
        driveType: '4x4',
        color: 'Crna',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1621007947382-bb3c3994e3fd?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 25,
        isNew: true,
        engineCc: 1598,
        fuelL100km: 1.4,
        electricRangeKm: 56,
        status: 'active'
    },
    {
        id: 'car-10',
        title: 'Renault Clio 1.0 TCe Expression',
        subtitle: '1.0 TCe Expression',
        price: '12.800 €',
        priceRaw: 12800,
        year: '2021',
        mileage: '32.000 km',
        mileageKm: 32000,
        power: '67 kW (91 KM)',
        powerKw: 67,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Prvi lastnik, malo kilometrov, redno servisiran.',
        location: { country: 'SI', region: 'Gorenjska' },
        seller: 'Anja Zupan',
        sellerImage: 'https://i.pravatar.cc/150?u=anja',
        sellerType: 'private',
        segment: 'B-hatchback',
        category: 'avto',
        make: 'Renault',
        model: 'Clio',
        variant: '1.0 TCe Expression',
        transmission: 'Ročni',
        driveType: 'Spredaj',
        color: 'Oranžna',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 7,
        isNew: false,
        engineCc: 999,
        fuelL100km: 5.2,
        status: 'active',
        equipment: [
            'Climate', 'Navigation', 'Bluetooth', 'CarPlay', 'ParkSensorRear', 
            'ABS', 'ESP', 'LED', 'DRL', 'CruiseControl', 'LaneAssist', 
            'ServiceBook', 'NonSmoking'
        ]
    },
    {
        id: 'moto-1',
        title: 'BMW R 1250 GS Adventure',
        subtitle: '1254 ccm, 100 kW (136 KM)',
        price: '21.400 €',
        priceRaw: 21400,
        year: '2023',
        mileage: '4.500 km',
        mileageKm: 4500,
        power: '100 kW (136 KM)',
        powerKw: 100,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Vsa možna oprema, kovčki vključeni. Kot nov.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Avtoval d.o.o.',
        sellerType: 'dealer',
        category: 'moto',
        subCategory: 'motorno-kolo',
        vtype: 'Adventure',
        make: 'BMW Motorrad',
        model: 'R 1250 GS',
        variant: 'Adventure',
        transmission: 'Ročni',
        color: 'Triple Black',
        images: {
            exterior: ['https://images.unsplash.com/photo-1558981403-c5f97cbba6c1?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 12,
        isNew: false,
        engineCc: 1254,
        engineStroke: '4T',
        engineType: 'Boxer',
        status: 'active'
    },
    {
        id: 'moto-2',
        title: 'Yamaha Tracer 9 GT',
        subtitle: '890 ccm, 87.5 kW (119 KM)',
        price: '13.900 €',
        priceRaw: 13900,
        year: '2024',
        mileage: '0 km',
        mileageKm: 0,
        power: '87.5 kW (119 KM)',
        powerKw: 87.5,
        fuel: 'Petrol',
        condition: 'Novo vozilo',
        sellerNote: 'Model 2024 na zalogi. GT oprema.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Moto Center Laba',
        sellerType: 'dealer',
        category: 'moto',
        subCategory: 'motorno-kolo',
        vtype: 'SportniTourer',
        make: 'Yamaha',
        model: 'Tracer 9',
        variant: 'GT',
        transmission: 'Ročni',
        color: 'Siva',
        images: {
            exterior: ['https://images.unsplash.com/photo-1591637333184-19aa84b3e01f?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 8,
        isNew: true,
        engineCc: 890,
        engineStroke: '4T',
        engineType: 'i3',
        status: 'active'
    },
    {
        id: 'moto-premium-1',
        title: 'BMW R 1250 GS Adventure',
        subtitle: '1254 ccm (136 KM) - Triple Black, Comfort, Touring',
        price: '24.800 €',
        priceRaw: 24800,
        year: '2023',
        mileage: '4.500 km',
        mileageKm: 4500,
        power: '100 kW (136 KM)',
        powerKw: 100,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Kot nov, vsa oprema. 3x original kovčki priloženi.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'BMW Motorrad Slovenija',
        sellerImage: 'https://i.pravatar.cc/150?u=bmw_moto',
        sellerType: 'dealer',
        segment: 'Enduro',
        category: 'motor',
        make: 'BMW',
        model: 'R 1250 GS',
        variant: 'Adventure',
        transmission: 'Ročni',
        driveType: 'Kardan',
        color: 'Črna',
        images: {
            exterior: [
                'images/bmw_gs.png'
            ],
            interior: []
        },
        imgCount: 12,
        isNew: false,
        isPremium: true,
        engineCc: 1254,
        engineStroke: '4T',
        engineType: 'Boxer',
        status: 'active'
    },
    {
        id: 'moto-2',
        title: 'Honda CB 650R Neo Sports Café',
        subtitle: '649 ccm (95 KM) - ABS, Traction Control, Quickshifter',
        price: '8.490 €',
        priceRaw: 8490,
        year: '2024',
        mileage: '10 km',
        mileageKm: 10,
        power: '70 kW (95 KM)',
        powerKw: 70,
        fuel: 'Petrol',
        condition: 'Novo vozilo',
        sellerNote: 'Novo vozilo v salonu. Možnost financiranja.',
        location: { country: "SI", region: "Savinjska" },
        seller: 'AS Domžale',
        sellerImage: 'https://i.pravatar.cc/150?u=honda',
        sellerType: 'dealer',
        segment: 'Naked',
        category: 'motor',
        make: 'Honda',
        model: 'CB 650R',
        variant: 'Neo Sports Café',
        transmission: 'Ročni',
        driveType: 'Veriga',
        color: 'Rdeča',
        images: {
            exterior: [
                'images/honda_cb.png'
            ],
            interior: []
        },
        imgCount: 8,
        isNew: true,
        isPremium: false,
        engineCc: 649,
        engineStroke: '4T',
        engineType: 'i4',
        status: 'active'
    },
    {
        id: 'com-premium-1',
        title: 'Mercedes-Benz Sprinter 317 CDI',
        subtitle: 'L3H2 - MBUX, Vzvratna kamera, Tempomat',
        price: '42.900 €',
        priceRaw: 42900,
        year: '2023',
        mileage: '15.000 km',
        mileageKm: 15000,
        power: '125 kW (170 KM)',
        powerKw: 125,
        fuel: 'Dizel',
        condition: 'Rabljeno',
        sellerNote: 'Odlično ohranjen, še v tovarniški garanciji.',
        location: { country: "SI", region: "Obalno-kraška" },
        seller: 'Autocommerce d.o.o.',
        sellerImage: 'https://i.pravatar.cc/150?u=mercedes',
        sellerType: 'dealer',
        segment: 'Dostavno vozilo',
        category: 'gospodarska',
        make: 'Mercedes-Benz',
        model: 'Sprinter',
        variant: '317 CDI L3H2',
        transmission: 'Avtomatski',
        driveType: 'Zadaj',
        color: 'Bela',
        images: {
            exterior: [
                'images/sprinter.png'
            ],
            interior: []
        },
        imgCount: 15,
        isNew: false,
        isPremium: true,
        engineCc: 1950,
        status: 'active'
    },
    {
        id: 'mach-1',
        title: 'John Deere 6120M',
        subtitle: 'Traktor — PowrQuad, klima, sprednja hidravlika',
        price: '68.500 €',
        priceRaw: 68500,
        year: '2019',
        engineHours: 3200,
        power: '88 kW (120 KM)',
        powerKw: 88,
        fuel: 'Dizel',
        condition: 'Rabljeno',
        sellerNote: 'Redno servisiran, garažiran, ena lastnica.',
        location: { country: 'SI', region: 'Podravska' },
        seller: 'Agro Tehnika d.o.o.',
        sellerImage: 'https://i.pravatar.cc/150?u=johndeere',
        sellerType: 'dealer',
        segment: 'Kmetijska mehanizacija',
        category: 'gospodarska',
        vehicleType: 'Traktor',
        bodyType: 'Traktor',
        make: 'John Deere',
        model: '6120M',
        variant: '6120M',
        transmission: 'Polavtomatski',
        color: 'Zelena',
        images: { exterior: [], interior: [] },
        imgCount: 8,
        isNew: false,
        engineCc: 4500,
        status: 'active'
    },
    {
        id: 'mach-2',
        title: 'Caterpillar 320 GC',
        subtitle: 'Gosenični bager — klimatska kabina, hitra spojka',
        price: '96.000 €',
        priceRaw: 96000,
        year: '2020',
        engineHours: 5400,
        power: '90 kW (122 KM)',
        powerKw: 90,
        fuel: 'Dizel',
        condition: 'Rabljeno',
        sellerNote: 'Delovno pripravljen, opravljen veliki servis.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Gradbena Mehanizacija d.o.o.',
        sellerImage: 'https://i.pravatar.cc/150?u=caterpillar',
        sellerType: 'dealer',
        segment: 'Gradbena mehanizacija',
        category: 'gospodarska',
        vehicleType: 'Bager (gosenični)',
        bodyType: 'Bager (gosenični)',
        make: 'Caterpillar',
        model: '320 GC',
        variant: '320 GC',
        transmission: 'Avtomatski',
        color: 'Rumena',
        images: { exterior: [], interior: [] },
        imgCount: 10,
        isNew: false,
        engineCc: 4400,
        status: 'active'
    },
    {
        id: 'car-11',
        title: 'Peugeot 3008 1.5 BlueHDi Allure',
        subtitle: '1.5 BlueHDi Allure (Navi, LED)',
        price: '22.500 €',
        priceRaw: 22500,
        year: '2021',
        mileage: '55.000 km',
        mileageKm: 55000,
        power: '96 kW (131 KM)',
        powerKw: 96,
        fuel: 'Diesel',
        condition: 'Rabljeno',
        sellerNote: 'Odličen družinski SUV, redno servisiran.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Avtohiša Lev',
        sellerImage: 'https://i.pravatar.cc/150?u=peugeot',
        sellerType: 'dealer',
        segment: 'SUV',
        category: 'avto',
        make: 'Peugeot',
        model: '3008',
        variant: '1.5 BlueHDi Allure',
        transmission: 'Avtomatski',
        driveType: 'Spredaj',
        color: 'Siva',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1549314467-f703e7aef7f3?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 14,
        isNew: false,
        engineCc: 1499,
        fuelL100km: 5.1,
        status: 'active'
    },
    {
        id: 'car-12',
        title: 'Kia Sportage 1.6 T-GDI LX Edition',
        subtitle: '1.6 T-GDI LX Edition',
        price: '28.900 €',
        priceRaw: 28900,
        year: '2023',
        mileage: '12.000 km',
        mileageKm: 12000,
        power: '110 kW (150 KM)',
        powerKw: 110,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Praktično novo vozilo, v garanciji še 6 let.',
        location: { country: 'SI', region: 'Savinjska' },
        seller: 'Tomaž Novak',
        sellerImage: 'https://i.pravatar.cc/150?u=tomaz',
        sellerType: 'private',
        segment: 'SUV',
        category: 'avto',
        make: 'Kia',
        model: 'Sportage',
        variant: '1.6 T-GDI LX Edition',
        transmission: 'Ročni',
        driveType: 'Spredaj',
        color: 'Zelena',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1563720223185-11003d516935?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 9,
        isNew: false,
        engineCc: 1598,
        fuelL100km: 6.8,
        status: 'active'
    },
    {
        id: 'car-13',
        title: 'Nissan Leaf 40 kWh Acenta',
        subtitle: 'Električno vozilo (40 kWh baterija)',
        price: '19.900 €',
        priceRaw: 19900,
        year: '2020',
        mileage: '45.000 km',
        mileageKm: 45000,
        power: '110 kW (150 KM)',
        powerKw: 110,
        fuel: 'Elektrika',
        condition: 'Rabljeno',
        sellerNote: 'Baterija v odličnem stanju, priložen hišni polnilec.',
        location: { country: 'SI', region: 'Podravska' },
        seller: 'E-Mobility d.o.o.',
        sellerImage: 'https://i.pravatar.cc/150?u=nissan',
        sellerType: 'dealer',
        segment: 'C-hatchback',
        category: 'avto',
        make: 'Nissan',
        model: 'Leaf',
        variant: '40 kWh Acenta',
        transmission: 'Avtomatski',
        driveType: 'Spredaj',
        color: 'Bela',
        doorsCount: 5,
        seatsCount: 5,
        images: {
            exterior: ['https://images.unsplash.com/photo-1593941707882-a5bba14938c7?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 11,
        isNew: false,
        engineCc: 0,
        electricRangeKm: 270,
        fuelL100km: null,
        status: 'active'
    },
    {
        id: 'moto-3',
        title: 'Kawasaki Z900',
        subtitle: '948 ccm (125 KM) - TFT zaslon, ABS',
        price: '9.200 €',
        priceRaw: 9200,
        year: '2022',
        mileage: '8.500 km',
        mileageKm: 8500,
        power: '92 kW (125 KM)',
        powerKw: 92,
        fuel: 'Petrol',
        condition: 'Rabljeno',
        sellerNote: 'Odlično ohranjen, kot nov.',
        location: { country: 'SI', region: 'Goriška' },
        seller: 'Moto Šport',
        sellerImage: 'https://i.pravatar.cc/150?u=kawa',
        sellerType: 'dealer',
        segment: 'Naked',
        category: 'motor',
        make: 'Kawasaki',
        model: 'Z900',
        variant: 'Standard',
        transmission: 'Ročni',
        driveType: 'Veriga',
        color: 'Zelena',
        images: {
            exterior: ['https://images.unsplash.com/photo-1558980664-2cd663cf8dde?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 6,
        isNew: false,
        engineCc: 948,
        engineStroke: '4T',
        engineType: 'i4',
        status: 'active'
    },
    {
        id: 'com-3',
        title: 'Renault Master Furgon L3H2',
        subtitle: '2.3 dCi (135 KM) - Nosilnost 1500kg',
        price: '21.500 €',
        priceRaw: 21500,
        year: '2020',
        mileage: '125.000 km',
        mileageKm: 125000,
        power: '99 kW (135 KM)',
        powerKw: 99,
        fuel: 'Dizel',
        condition: 'Rabljeno',
        sellerNote: 'Pripravljen za delo, nedavno opravljen veliki servis.',
        location: { country: 'SI', region: 'Osrednjeslovenska' },
        seller: 'Gospodarska vozila d.o.o.',
        sellerImage: 'https://i.pravatar.cc/150?u=renaultcom',
        sellerType: 'dealer',
        segment: 'Dostavno vozilo',
        category: 'gospodarska',
        make: 'Renault',
        model: 'Master',
        variant: 'Furgon L3H2',
        transmission: 'Ročni',
        driveType: 'Spredaj',
        color: 'Bela',
        images: {
            exterior: ['https://images.unsplash.com/photo-1626025219901-384392f4da3b?auto=format&fit=crop&w=800&q=80']
        },
        imgCount: 8,
        isNew: false,
        engineCc: 2299,
        status: 'active'
    }
];

// Platform-aware demo/fallback listing set — DEV only.
import { sampleBoats } from './sampleBoats.js';
import { PLATFORM as _PLATFORM } from '../config/platform.js';
export const SAMPLE_LISTINGS = import.meta.env.DEV
    ? (_PLATFORM.id === 'navtika' ? sampleBoats : sampleCars)
    : [];
